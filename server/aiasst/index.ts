...
const createTrigger = async (userId?: string) => {
  ...
  if ((await totalTrigger) >= USER_MAX_TRIGGERS)
    throw new Error(
      `Your triggers have reached the maximum limit: ${USER_MAX_TRIGGERS}`
    );
  const now = new Date();
...
const getTriggerList = async (userId: string) => {
  if (!userId) return [];
  try {
    return await AiAsstTrigger.find(
      { user: userId },
      { _id: 1, name: 1, enbl: 1, intv: 1 }
    ).lean();
  } catch {}
  return [];
};
...
const getTrigger = async (userId: string, _id: string) => {
  ...
  return trigger
    ? ({ ...trigger, _id: trigger?._id.toHexString() } as Trigger)
    : null;
};
...
const execGooglePlugin = async (query?: string) => {
  ...
  const res = await fetch(
    `https://api.cch137.link/google-search-summary?query=${query}`
  );
...
const execCrawlPlugin = async (url?: string) => {
  url = (url || "").trim();
  if (!url) return "";
  if (/^https?:\/\/api\.cch137\.link(\/|$)/.test(url))
    return await (await fetch(url)).text();
  const res = await fetch("https://api.cch137.link/crawl-text", {
    method: "POST",
...
const execPlugins = async (
  plugins: string[] | PluginObject[]
): Promise<string[]> => {
  return await Promise.all(
    plugins.map((_plugin) => {
      try {
        const { type, args } =
          typeof _plugin === "string"
            ? (JSON.parse(_plugin) as PluginObject)
            : _plugin;
...
const testTrigger = async (userId: string, _id: string) => {
  ...
  return aiProvider.ask(
    ...
    trigger.modl as SupportedModel
  );
};
...
const execTrigger = async (trigger: Trigger) => {
  ...
  try {
    const stream = aiProvider.ask(
      ...
      modl as SupportedModel
    );
    await stream.untilDone;
    mailer.sendHtml(
      eadd,
      `${trigger.name} - ${appName}`,
      await marked.parse(stream.read())
    );
    log = `Success at ${formatDate(new Date())} (GMT+0) took ${
      Date.now() - t0
    } milliseconds to execute by ${modl}.`;
  } catch (e) {
    log = `Error (${
      e instanceof Error ? e.message : "Unknown"
    }) at ${formatDate(new Date())} (GMT+0) took ${
      Date.now() - t0
    } milliseconds to execute by ${modl}.`;
  } finally {
    const nextsche = calcNextSche(strt, intv);
