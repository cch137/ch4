import type Stream from "@cch137/utils/stream";
...
export class SuperProvider<
  Providers extends { [model: string]: BaseProvider }
> {
  readonly #handlers = new Set<SuperProviderResHandler>();
  readonly providers: Providers;
  get defaultModel(): keyof Providers {
  ...
  ask(options: UniOptions | string, _defaultModel?: keyof Providers) {
    if (typeof options === "string")
      return this.ask(wrapOptions(options), _defaultModel);
    const model = (
      options.model ||
      _defaultModel ||
      this.defaultModel
...
export { GeminiProvider, OneApiProvider };
