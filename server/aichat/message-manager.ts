...
const getConvList = async (userId?: string) => {
  try {
    ...
    return await AiChatConversation.find(
      { user: userId },
      { _id: 0, id: 1, name: 1, mtms: 1 }
    );
  } catch {
    return [];
  }
...
const isOwnerOfConv = async (userId?: string, convId?: string) => {
  ...
  return Boolean(
    await AiChatConversation.findOne({ user: userId, id: convId }, { _id: 1 })
  );
};
...
const setConv = async (
  userId?: string,
  convId?: string,
  data?: { name?: string; conf?: string; tail?: string }
) => {
...
const accessedConv = async (userId?: string, convId?: string) => {
  if (!userId) return { success: false, message: "UserId is required" };
  if (!convId) return { success: false, message: "ConvId is required" };
  return await ConvOp.find(userId).set("atms", Date.now()).save();
};

const modifiedConv = async (userId?: string, convId?: string) => {
  if (!userId) return { success: false, message: "UserId is required" };
  if (!convId) return { success: false, message: "ConvId is required" };
  return await ConvOp.find(userId).set("mtms", Date.now()).save();
};
...
const createConv = async (userId?: string): Promise<StatusResponse<string>> => {
  if (!userId || !(await userManager.hasUserById(userId)))
...
const insertMessage = async (
  ...
  if (!userId || !conv || !(await isOwnerOfConv(userId, conv)))
    return { success: false, message: "Conversation is requried" };
  const ctms = Date.now();
...
const setMessage = async (
  ...
  if (!(await isOwnerOfConv(userId, convId)))
...
const delMessage = async (
  ...
  if (!userId || !convId || !_id)
    return { success: false, message: "Message not found" };
  if (typeof _id !== "string")
    return await delMessage(userId, convId, _id.toHexString());
  if (!(await isOwnerOfConv(userId, convId)))
    return { success: false, message: "Not owner" };
  ...
  try {
    await Promise.all([
      ...
      AiChatMessage.updateMany(
        { root: _id },
        root ? { $set: { root } } : { $unset: { root: 1 } }
      ),
      AiChatConversation.updateOne(
        { tail: _id },
        root ? { $set: { tail: root } } : { $unset: { tail: 1 } }
      ),
    ]);
    return { success: true };
  } catch {
