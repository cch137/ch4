...
import { IoArrowUpCircleOutline, IoShuffle } from "react-icons/io5";
...
export default function Laundry() {
  const [files, setFiles] = useState<
    { name: string; size: number; buffer: Uint8Array }[]
  >([]);
  ...
  const convertFiles = async () => {
    ...
    try {
      if (Date.now() - lastConvertedAt.current > 10 * 60000) {
        ...
        taskId.current = (
          await (
            await fetch(`${origin}/images-to-pdf/create-task`, {
              method: "POST",
            })
          ).json()
        )?.id;
        if (typeof taskId.current !== "string")
          throw new Error("Failed to create task");
        setMessage(`created task: ${taskId.current}`);
        let done = 0;
        const { length } = files;
        await Promise.all(
          files.map(async ({ buffer }, i) => {
            await fetch(
              `${origin}/images-to-pdf/upload/${taskId.current}/${i}`,
              {
                method: "POST",
                body: buffer,
      ...
      const res = await fetch(
        `${origin}/images-to-pdf/convert/${taskId.current}/${_filename}`,
        { method: "POST" }
      );
      if (!res.body) return;
  ...
  return (
    <div className="max-w-2xl m-auto py-8 px-4">
      <div className="flex-center">
        <h1 className="flex-4 text-2xl text-default-600 font-bold">
          Images to PDF
        </h1>
      </div>
      <Spacer y={8} />
      ...
      <div className="flex-center py-4 text-default-300">{message}</div>
      {files.length === 0 ? null : (
      ...
      <div className="flex items-end justify-center">
        ...
        <div className="text-sm text-default-300 font-bold">
          total: {files.length}
        </div>
      </div>
