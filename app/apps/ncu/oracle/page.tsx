...
function CharRow({
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        setSources([]);
        const res: ImageSource[] =
          cache.get(char) ||
          (await (
            await fetch(`${apiOrigin}/ccamc-images`, {
              method: "POST",
              body: `q=${char}`,
            })
          ).json());
  ...
  return (
    <div className="p-4 border-1 border-solid border-default-300 rounded-xl">
      <div className="flex">
...
export default function Oracle() {
  ...
  const getInfer = useCallback(
    async (q: string | null) => {
      if (!q) return [];
      setIsAddingInfer(true);
      try {
        const infer: InferSource = await (
          await fetch(`${apiOrigin}/ccamc-infer`, {
            method: "POST",
            body: `q=${q}`,
          })
        ).json();
  ...
  return (
    <div className="px-4 py-8 max-w-screen-lg m-auto">
      <div className="flex gap-4">
        ...
        <Button
          onClick={() =>
            getInfer(prompt("關鍵字:")).then((infer) => addChars(infer))
          }
          isLoading={isAddingInfer}
        >
          新增建議
        </Button>
      ...
      <div className="flex flex-col gap-4 py-4">
        {chars.map((char, i) => (
          <CharRow
            char={char}
            removeChar={() => setChars((_) => _.filter((v) => v !== char))}
            key={i}
          />
        ))}
      </div>
