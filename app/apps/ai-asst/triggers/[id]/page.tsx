...
import { IoAddOutline, IoChevronBackOutline, IoCreateOutline, IoPlay, IoTrashOutline } from "react-icons/io5";
...
function PluginItem({
  ...
  return (<>
    <div className="flex gap-2 bg-default-50 rounded-xl px-4 py-2 w-full">
        ...
        <div className="relative">
          <div className="absolute w-full text-default-300 text-nowrap text-ellipsis overflow-hidden">
            {args.map((item) => {
              ...
              return `${name}: ${(value.length || 0) > 64 ? `${value.substring(0, 64)}...` : value}`;
            }).join('; ')}
          </div>
        </div>
      </div>
      <div className="flex gap-1 bg-inherit">
        <Tooltip content="Edit" placement="right">
          <div onClick={onOpen} className="flex-center p-1 bg-inherit rounded-lg cursor-pointer hover:brightness-150 transition text-lg">
            <IoCreateOutline />
          </div>
        </Tooltip>
      </div>
    ...
    >
      <ModalContent>
        {(onClose) => (<>
          <ModalHeader className="flex flex-col gap-1">Edit Plugin</ModalHeader>
          <ModalBody>
            <div className="flex-center">
              <Select
                ...
                onChange={(e) => setPlugin(o => ({...o, type: e.target.value as PluginType}))}
              >
                <SelectItem value="text" key="text">Plain Text</SelectItem>
                <SelectItem value="google" key="google">Google Search</SelectItem>
                <SelectItem value="time" key="time">Time</SelectItem>
                <SelectItem value="crawl" key="crawl">Crawl URL</SelectItem>
              </Select>
              <div className="flex-1" />
              <div>
                <Tooltip content="Delete">
                  <Button onClick={deletePlugin} color="danger" variant="flat" isIconOnly>
    ...
    <div className="flex flex-col gap-4 py-4">
      <div className="flex items-center flex-wrap gap-4">
        ...
        <Select
          ...
          onChange={(e) => setForm(o => ({...o, enbl: Boolean(Number(e.target.value))}))}
        >
          <SelectItem value={1} key={1}>ON</SelectItem>
          <SelectItem value={0} key={0}>OFF</SelectItem>
        </Select>
        <div className="flex-1" />
        <div className="text-sm text-default-400 select-none">
          {isSaving ? <div className="flex-center gap-2">
      ...
      <div className="flex flex-col gap-1">
        <div className="text-default-500">
          When this trigger is activated, an email will be sent to your account mailbox.
        </div>
      ...
      {isNaN(nextExec.getTime()) ? null : <div className="text-default-300 text-sm">
      ...
      <div className="max-w-2xl">
        <div className="flex gap-4">
          ...
          <div className="flex-center full">
            <Button
              onClick={() => form.plug.push({type: 'text', args: [''], isNew: true})}
              variant="light"
              size="sm"
            >
        ...
        <div className="flex flex-col gap-2 py-2">
          {pluginsMirror.map((p, i) => <PluginItem index={i} plugins={form.plug} key={i} />)}
        </div>
      </div>
      <Spacer y={2} />
      <Divider />
      <div className="flex flex-col gap-2">
        <div className="flex items-end">
          ...
          <div className="text-sm text-default-300">Note: Test results will not be sent to your mailbox.</div>
        </div>
        <div>
          <Button
            startContent={(isTesting || isSaving) ? void 0 : <IoPlay className="text-lg" />}
            size="sm"
            variant="faded"
          ...
          >
            <span className="text-sm pr-2">{isSaving ? 'Saving...' : isTesting ? 'Executing...' : 'Test'}</span>
          </Button>
        </div>
        {execResult ? <Textarea
          label="Test result"
