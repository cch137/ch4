...
export const createTrigger = async () => {
  try {
    const { success, value, message }: StatusResponse<string> = await (
      await fetch("/api/ai-asst/triggers/", { method: "PUT" })
    ).json();
    if (!success || !value)
      throw new Error(message || "Failed to create trigger");
...
export const deleteTrigger = async (_id?: string) => {
  try {
    ...
    const { success, message }: StatusResponse<string> = await (
      await fetch(`/api/ai-asst/triggers/${_id}`, { method: "DELETE" })
    ).json();
...
export const updateTrigger = async (trigger?: Trigger) => {
  try {
    ...
    const { success, message }: StatusResponse = await (
      await fetch(`/api/ai-asst/triggers/${trigger._id}`, {
        method: "PUT",
...
export const triggersStore = store<TriggerItem[]>(
  [],
  async () => {
    try {
      return await (
        await fetch("/api/ai-asst/triggers", { method: "POST" })
      ).json();
    } catch {
      return [];
    }
