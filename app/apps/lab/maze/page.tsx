...
class Maze {
  ...
  constructor(seed: number = 0, size: number) {
    ...
    while (true) {
      ...
      while (true) {
        const siblings = lastAddedTile.siblingWallNodes.filter(
          (s) => !s.isWall
        );
    ...
    // Merge short walls
    while (true) {
      const shortWalls: MazeTileGroup[] = [...walls].filter(
        (w) => w.size < size / 4
      );
...
class MazeTile {
  ...
  get siblingWallNodes() {
    return this.isWallNode ? this.siblingNodes : [];
  }

  get siblingPathNodes() {
    return this.isPathNode ? this.siblingNodes : [];
  }
}
...
class MazeWalker {
  ...
  getTileStatus() {
    ...
    if (isSuccess)
      status.forEach((s, t) => (s <= 1 ? status.set(t, -1) : null));
...
class MazeBranch {
  ...
  forward() {
    ...
    const nexts = walked
      .at(-1)!
      .neighbours.filter((t) => !t.isWall && !walked.includes(t));
    const { length } = nexts;
  ...
  getTileStatus(targetLength: number) {
    ...
    const isDeadEnd =
      walked.length !== targetLength ||
      this.walker.branches.find(
        (b) =>
          b !== this &&
          b.walked.includes(headTile) &&
          headTile !== b.walked.at(-1)
      );
    if (isDeadEnd) {
      this.dead = true;
      walked.forEach((t) => status.set(t, -1));
...
function Tile({
  ...
  return (
    <div
      className={["relative maze-tile", isWall ? "wall" : "path cursor-pointer"]
        .join(" ")
        .replace(/\s+/g, " ")}
      onMouseOver={
        isWall
...
export default function MazeLab() {
  ...
  const createMazeWalker = useCallback(
    (maze: Maze) => {
  ...
  const generateMaze = useCallback(
    (_seed: number = seed, _size: number = size) => {
  ...
  return inited.current ? (
    <div className="flex justify-start items-start">
      <div className="flex-center flex-col p-8 gap-4 m-auto">
        ...
        <div
          className={`flex text-stone-600 transition ${
                ...
                >
                  {c.map((tile, y) => (
                    <Tile
                      tile={tile}
                      showMark={showMark}
                      clearMarkSymbol={clearMarkSymbol}
                      key={y}
                      status={status ? status.get(tile) : void 0}
                    />
                  ))}
                </div>
              ))}
