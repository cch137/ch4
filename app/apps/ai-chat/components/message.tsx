...
import { ClassAttributes, HTMLAttributes, createRef, useCallback, useState } from "react";
...
function MessageA({
  ...
  return <a
    ...
    href={props.href}
    target="_blank"
    className={'underline opacity-75 decoration-default-500 hover:opacity-90 transition ' + className}
    {...props}
  >
    ...
    <div className="aichat-message w-full p-2 pb-0">
      <div className={isModel ? 'text-default-600' : 'text-default-800'}>{
        ...
        ? (
          ...
          <Markdown components={{code: MessageCodeBlock, a: MessageA}} remarkPlugins={[remarkGfm]} >{text}</Markdown>
          // isModel
      ...
      <div className="flex justify-end items-center flex-wrap pt-2">
        {disableActions ? null : <div className="flex text-lg gap-2 aichat-message-actions">
          ...
          <Tooltip content="Copy" placement="bottom" showArrow>
            <div
              onClick={copyText}
              className={copied ? 'flex-center text-success-500 opacity-100' : ''}
        ...
        {(cLength > 1) ? (
          <div className="text-sm px-2 flex-center select-none">
            <span onClick={() => message.gotoSibling(-1)} className={`cursor-pointer font-semibold px-1 ${cIndex === 0 ? 'opacity-25' : 'opacity-75'}`}>{'<'}</span>
            <span className="text-default-500 font-base">{cIndex + 1}/{cLength}</span>
            <span onClick={() => message.gotoSibling(+1)} className={`cursor-pointer font-semibold px-1 ${cIndex >= cLength - 1 ? 'opacity-25' : 'opacity-75'}`}>{'>'}</span>
          </div>
        ) : null}
        <div className="flex-1 flex justify-end items-center font-semibold text-default-200 gap-2 text-xs select-none whitespace-nowrap">
          ...
          {(ctms) ? <span className="text-default-300">{formatDate(new Date(ctms), 'yyyy/MM/dd HH:mm')}</span> : null}
        </div>
      </div>
    </div>
  </>
