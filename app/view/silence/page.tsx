...
class AudioSource {
  ...
  get mainUrl(): string {
    return AudioSource.getUrl("main", this.id);
  }
  get glueUrl(): string {
    return AudioSource.getUrl("glue", this.id);
  }
}
...
const catrgorizedSources = (() => {
  ...
  const categorized: AudioCategory[] = categories.map((name) => ({
    name,
    sources: [],
  }));
...
const isAlmostEnd = ({ duration, currentTime }: HTMLAudioElement) =>
...
function AudioController({
  ...
  return (
    <div
      className="w-40 text-default-500 transition"
      style={{ filter: `brightness(${0.75 + volume * 0.5})` }}
    >
...
export default function Silence() {
  ...
  const loadConfig = useCallback(
    (mix: MixConfig) => {
      ...
      sources.forEach((source) => {
        const savedSource = mix.sources.find(
          ({ id: _id }) => _id === source.id
        );
  ...
  useInit(() => {
    const loadedMixes = (() => {
      try {
        const mixes: MixConfig[] = JSON.parse(
          localStorage.getItem(LOCALSTORAGE_KEY)!
        );
        if (!Array.isArray(mixes) || mixes.length < 1)
          throw new Error("No Saved Mixes");
        return mixes;
      } catch {
  ...
  useEffect(() => {
    ...
    document.addEventListener("wheel", controlVolumeByWheel, {
      passive: false,
    });
    ...
    <>
      ...
      <div className="px-4 py-8 m-auto select-none" style={{ maxWidth: 960 }}>
        ...
        <div className={`${serifFontClassname} flex flex-col gap-4`}>
            ...
            <div className="flex flex-col gap-3">
              ...
              >
                ...
                <div className="text-sm">
                  {isPlaying ? "播放中" : "已暫停"}：
                  {mixConfigList.find((m) => m.isPlaying)?.name || "-"}
                </div>
              </div>
            ...
            <div className="flex-1">
              <Accordion isCompact defaultSelectedKeys="all">
                <AccordionItem
                  aria-label="Mixes"
                  title="我的混錄"
                  classNames={{ title: "text-default-500" }}
                >
                      ...
                      <div className="flex" key={i}>
                        ...
                        <div className="flex-center w-7 overflow-hidden opacity-75">
                          <Button
                            ...
                            onClick={() => {
                              if (
                                mixConfigList.length > 1 &&
                                confirm(`是否刪除混錄：${mix.name}`)
                              )
                                setMixConfigList(
                                  mixConfigList.filter((m) => m !== mix)
                                );
                            }}
                          >
                            <MdDeleteForever />
          ...
          <Spacer />
          <Divider />
          {catrgorizedSources.map((cate, i) => (
            <div key={i}>
              <div className="text-2xl text-default-500 font-semibold">
                {cate.name}
              </div>
              <div className="flex flex-wrap gap-x-4 gap-y-2 p-1">
                {cate.sources.map((s, i) => (
                  <AudioController
                    audio={s}
                    currentMix={mixConfigList.find((m) => m.isPlaying)!}
                    globalVolume={globalVolume}
                    speed={isPlaying ? globalSpeed : 0}
                    key={i}
                  />
                ))}
              </div>
          ...
          <div className="flex-center flex-col gap-4 py-16">
            <Button
              ...
              variant="bordered"
              onClick={() => setShowRelaxingMusic((v) => !v)}
            >
