...
export default function ConversationConfig({
  ...
  const adjustModelSettingsElHeight = useCallback(async () => {
    setTimeout(async () => {
      setModelSettingsHeight(`${(document.getElementById(MODEL_SETTINGS_ID) as HTMLDivElement).offsetHeight}px`);
    }, 0);
  ...
  }, [inited, setSelectedModel, setIsFetchingModels, adjustModelSettingsElHeight, models]);
    ...
    <div className="flex flex-col gap-2">
      <div className="flex gap-2">
        ...
        <div className="flex-1 gap-1 flex-center">
          <Tooltip content="Model settings" placement="right" isDisabled={modelSettingOpened}>
            <Button
              isIconOnly
              color="secondary"
            ...
            >
              {modelSettingOpened ? <IoChevronUp style={({ scale: 4 / 3 })} /> : <IoSettingsSharp style={({ scale: 4 / 3 })} />}
            </Button>
          </Tooltip>
          {isSmallScreen ? (
            <>
              <div className="flex-1"></div>
              <Button isIconOnly color="danger" variant="light" onClick={closeSidebar}>
      ...
      <div
        ...
        style={({ transition: ".2s ease-out", height: modelSettingOpened ? modelSettingsHeight : '0px', opacity: modelSettingOpened ? '100%' : '0%' })}
      >
