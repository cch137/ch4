  ...
  const refresh = useCallback(async () => {
    try {
      ...
      const res = await fetch("/api/auth/online", {
        method: "POST",
        body: JSON.stringify({ lastMs }),
      });
      const stream = await readStream(res.body);
      try {
        const list = unpackDataWithHash<UserOnlineState[]>(
          stream,
          256,
          371117069,
          1681681688
        ...
        const { message }: StatusResponse = JSON.parse(
          new TextDecoder("utf-8").decode(stream)
        );
        throw new Error(
          message ||
            (e instanceof Error ? e.message : "Failed to fetch online users.")
        );
      }
    } catch (e) {
        ...
        <Table>
          ...
          <TableBody emptyContent={"No Users"}>
            {userList.map((v, i) => (
              <TableRow
                key={i}
                className="bg-default-50 hover:brightness-150 transition"
              >
