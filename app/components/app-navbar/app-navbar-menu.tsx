...
export default function AppNavbarMenu() {
  ...
  const isSignInPage = pathname === SIGNIN_PATHNAME;
  ...
  return (
    <>
      ...
      {$inited && isSignedIn ? (
        <Avatar
          ...
          className={`transition-transform p-0 ${
            menuIsOpen ? "opacity-75 scale-95" : ""
          }`}
          color="secondary"
          size="sm"
          icon={<AvatarIcon />}
      ...
      <div
        ref={menuRef}
        className={`app-navbar-menu shadow-2xl w-52 p-2 rounded-2xl border-1 border-default-50 bg-zinc-950 absolute ${
          menuIsOpen ? "" : "close"
        }`}
        ...
        >
          <div className="w-full flex items-center gap-1.5">
            <Avatar
              ...
              icon={<AvatarIcon />}
            />
            <div className="font-semibold text-base w-32 truncate text-start">
              {username}
            </div>
          </div>
        </Button>
