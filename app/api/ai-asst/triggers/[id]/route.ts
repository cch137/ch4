...
export async function POST(req: NextRequest, context: NextApiContext) {
  ...
  try {
    ...
    return new NextResponse(packDataWithHash(trigger, 256, 721663210, 20240202));
  } catch {
...
export async function PUT(req: NextRequest, context: NextApiContext) {
  ...
  const triggerId = context?.params?.id || '';
  try {
    const trigger = unpackDataWithHash<Trigger>(await readStream(req.body), 256, 721663210, 20240202);
    ...
    return NextResponse.json({success: false, message: 'Failed to update trigger'});
  }
}
...
export async function DELETE(req: NextRequest, context: NextApiContext) {
  ...
  try {
    ...
    return NextResponse.json({success: false, message: 'Failed to delete trigger'});
  }
}
