...
const rateLimiter = new RateLimiter([{ maxCount: 5, timeMs: 60000 * 30 }]);
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const res = NextResponse.json(
      await auth.userManager.setEmail(token.id, eadd)
    );
...
export async function POST(
  ...
  if (!rateCheck.success) return NextResponse.json(rateCheck, { status: 429 });
  try {
    const { action, eadd } = unpackData<{ action: number; eadd: string }>(
    ...
    switch (action) {
      case 0: // has
        ...
        return NextResponse.json({
          success: isVerifying,
          message: isVerifying ? undefined : "Not verifying",
        });
      case 1: // create
