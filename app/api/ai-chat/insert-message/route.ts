...
import type { MssgItem, SaveMssg, SaveMssgRes } from "@/constants/chat/types";
...
export async function POST(req: NextRequest): Promise<NextResponse<StatusResponse<MssgItem>>> {
  ...
  try {
    ...
    // check version
    if (parse(msg.vers).lt([0,6,3])) return NextResponse.json({ success: false, message: 'Version oudated, please reload.' });
