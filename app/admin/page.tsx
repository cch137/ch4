...
const resetEmitter = new Emitter<{ reset: [] }>();
...
function AdminItemInput<K extends string, V = any>({
  name,
  value,
  isDisabled,
}: {
  ...
  const initStringifyValue = JSON.stringify(
    adminStore.config.find((i) => i[0] === name)![1]
  );
  ...
  const resetValue = useCallback(
    () => setV(checkValue(initStringifyValue)),
    [setV, initStringifyValue, checkValue]
  );
  useEffect(() => {
    resetEmitter.on("reset", resetValue);
    return () => {
      resetEmitter.off("reset", resetValue);
    };
  }, [resetValue]);
  ...
  return (
    <div>
      ...
      {warnMessage ? (
        <div className="text-sm text-warning-400">{warnMessage}</div>
      ) : null}
      {dangerMessage ? (
        <div className="text-sm text-danger-400">{dangerMessage}</div>
      ) : null}
...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    adminErrorEmitter.on("error", handler);
    return () => {
      adminErrorEmitter.off("error", handler);
    };
  }, [openErrorMessageBox]);
  ...
  return (
    <>
      ...
      {!admin.isLoggedIn ? (
        ...
        <>
          <div
            className="flex flex-col gap-4 p-8 pb-32 m-auto"
            style={{ maxWidth: 680 }}
          >
            ...
            {admin.config.map(([k, v]) => (
              <AdminItemInput
                name={k}
                value={v}
                isDisabled={admin.isUpdating}
                key={k}
              />
            ))}
          </div>
